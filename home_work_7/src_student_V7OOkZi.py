...
def gini(y: np.ndarray) -> float:
    ...
    res = np.zeros(n)
    for i in range(n):
        res[i] = np.count_nonzero(y == values[i]) / y.shape[0]
    gini_vec = res * (1 - res)
    return gini_vec.sum()
...
def weighted_impurity(y_left: np.ndarray, y_right: np.ndarray) -> \
    ...
    weighted_impurity = (left_impurity * y_left.shape[0] + right_impurity * y_right.shape[0]) / (y_left.shape[0] + y_right.shape[0])
...
class MyDecisionTreeNode:
    ...
    def _best_split(self, X: np.ndarray, y: np.ndarray):
        ...
        for feature in features:
            ...
            for threshold in thresholds:
                ...
                current_weighted_impurity, current_left_impurity, current_right_impurity = weighted_impurity(current_feature_values[left_idx], current_feature_values[right_idx])
                # calcualte impurity for given indices
    ...
    def fit(self, X: np.ndarray, y: np.ndarray):
        ...
        if (
            ...
            self._class_proba = ... # compute its probability as ratio
            return self
