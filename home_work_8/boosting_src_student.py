...
class MyBinaryTreeGradientBoostingClassifier:
    ...
    *Binary* gradient boosting with trees using
    ...
    def cross_entropy_loss(
        ...
        :return:
        """
        return -sum(true_labels * np.log(expit(logits)) + (1 - true_labels) * np.log(1 - expit(logits)))
    ...
    def cross_entropy_loss_gradient(
        ...
        :return:
        """
        return expit(logits) - true_labels
    ...
    def fit(
        ...
        self.initial_logits = logit(y.mean())
        # create starting logits
        logits = self.initial_logits * np.ones(y.shape[0])
        ...
        for seed in self.rng.choice(
            ...
            self.estimators.append(self.create_new_estimator(seed))
            # compute gradient
            gradient = MyBinaryTreeGradientBoostingClassifier.cross_entropy_loss_gradient(y, logits)
            # fit estimator on gradient residual
            self.estimators[-1].fit(X, -gradient)
            # adjust logits with learning rate
            logits = logits - self.learning_rate * logit(self.estimators[-1].predict(X))
